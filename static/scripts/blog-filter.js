document.addEventListener('DOMContentLoaded', () => {
    ...
    const updatePostsCount = (doc) => {
        const newCount = doc.querySelector('.post-count')?.textContent;
        if (!newCount) return;
    ...
    const fetchPosts = async () => {
        ...
        try {
            ...
            DOM.postsContainer.innerHTML = doc.querySelector('#post-list')?.innerHTML || '';
            DOM.paginationWrapper.innerHTML = doc.querySelector('#pagination-wrapper')?.innerHTML || '';

            updatePostsCount(doc);
    ...
    const fetchPostsDebounced = debounce(fetchPosts, 400);
