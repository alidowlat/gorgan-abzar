...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function validateInput(value) {
        ...
        if (persianDigits.test(value)) {
            return "شماره موبایل باید با اعداد انگلیسی باشد";
        }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (timerElement && resendButton && timerTextElement && submitButton) {
        let time = 120;
        let timerInterval;
        ...
        resendButton.addEventListener('click', function () {
            if (!resendButton.classList.contains('active')) return;
        ...
        otpInputs.forEach(input => {
            input.addEventListener('input', checkOtpInputs);
            ...
            input.addEventListener('keydown', (e) => {
                if (e.key === "Enter") {
                    e.preventDefault();
                    if (Array.from(otpInputs).every(inp => inp.value.trim())) {
                        submitButton.click();
                    }
                }
            });
